  ...
  <div className="row">
      ...
      <SideBar active={props.active} type="driver"/>
        ...
        <div className="col-md-9">
            ...
            <Link to="/driver/dashboard" className="btn btn-lg rounded-pill btn-primary text-white "><i className="fas fa-chevron-left"></i> Back</Link>
             <div className="pc fs-29 fw-700 py-3">Jobs</div>
          <div className="card card-body green-2-bg ">

              <div className="pc fs-24 fw-700 pb-4">Current Jobs in {currentLocation.name}</div>
              <div className="row g-4">

                <div className="col-md-12">

           {isReady ? <Loading2 /> : jobs.jobs.filter((item) => item.type === "intra-state" && item.status === "pending" && item.from_city.state_id === currentLocation.id).length === 0 ? <div className="alert alert-info">No jobs is available in {currentLocation.name}</div> :

           jobs.jobs.slice(0,10).map((item) => item.type == "intra-state" && item.status === "pending" && item.from_city.state_id === currentLocation.id ? 
                   ...
                   {isReady ? "Loading" : jobs.jobs.filter((item) => item.type === "intra-state" && item.status === "pending" && item.from_city.state_id === currentLocation.id).length !== 0 ? <div className="text-center mt-4"><a href="#" className="fw-18 pc fw-700">View All</a></div> : null}
